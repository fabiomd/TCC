	...
	(let ((sub-webassembly-symbols-table (copy-webassembly-symbols-table webassembly-symbols-table))
	    ...
	    (if (deeper node)
	    	(if (eql (type-of node) 'operator-node)
	    		(let ((sub-nodes-formated (get-operator-parameters-formated node)))
	    			(print T)
	    			(let ((chosen (choose sub-nodes-formated)))
	    				(print "chosen1")
	    				(print chosen)
	    	...
	    	(progn (print nil)
		    (apply-action-node sub-nodes action sub-webassembly-symbols-table)
		    )
	    )
    )
)
...
(defun apply-action-node (available-nodes action webassembly-symbols-table)
	(print "CALLED")
	(print action)
