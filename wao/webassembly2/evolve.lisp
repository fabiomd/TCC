...
; (defmethod size (listL)
; 	 (list-length listL)
; 	)

; (defmethod size ((software software)) (length (genome software)))

; (defmethod lines ((wasm wasm-perf))
; 	(wasm-perf-genome wasm)
; 	)
...
; (defun pick-good ((wasm wasm-perf))
; 	(get-genome-at-position (random (size (wasm-perf-genome wasm))))
; 	)
...
(defun crossover (wasm-perf-a wasm-perf-b)
	(print wasm-perf-a)
	(print wasm-perf-b)
; (replace '(a b c d e) '(x) :start1 3)
	)

(defun mutate (wasm-perf-a)
	(print wasm-perf-a)
)

(defmethod fitness (wasm-perf-a wasm-perf-b)
	(print wasm-perf-a)
	; (print restargs)
	(random 100)
	)

; (defun fitness (wasm-perf-a wasm-perf-b)
; 	(print wasm-perf-a)
; 	(print wasm-perf-b)
; 	(random 100))
...
(defun tournament
  ...
  (flet ((verify (it)
           (assert (typep it 'wasm-perf) (it)
                   "Population member is not software object")
           (assert (numberp (wasm-perf-fitness it)) ((wasm-perf-fitness it))
